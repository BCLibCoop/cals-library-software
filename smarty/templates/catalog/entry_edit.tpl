...
<input type="button" value="{$labels['newFieldButtonLabel']}" onclick="self.location='entry_marc_new.php?bid={$bibid}'" />&nbsp;&nbsp;
<input type="button" value="{$labels['cancelButtonLabel']}" onclick="self.location='entry_view.php?bid={$bibid}'" />&nbsp;&nbsp;
<input type="hidden" name="bid" value="{$bibid}"/>
<input type="hidden" id="chmid" name="chmid" value="">
<font>{$labels.requiredWarning}</font>
...
{foreach from=$fields.general key="tidx" item="part"}
	...
	<td width="8%" align="center"  class="left">{if $generalButtons[$tidx]}<input type="button" value="{$tidx}" onclick="self.location='entry_marc_edit.php?f={$part.field->getFieldid()}'"/>{else}{$tidx}{/if}</td>
	<td width="35%" >{if $part.field->getIsRequired()}<sup>*</sup>{/if}{$part.desc}</td>
...
<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	...
	<td width="50%" class="right"><input type="textfield" name="callno1" value="{$entry->getCallNmbr1()}" size="20" maxlength="20" /><br/>
	<input type="textfield" name="callno2" value="{$entry->getCallNmbr2()}" size="20" maxlength="20" /><br/>
	<input type="textfield" name="callno3" value="{$entry->getCallNmbr3()}" size="20" maxlength="20" /></td>
	{assign var="rowidx" value=$rowidx+1}
</tr>
...
<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	<td class="left"></td>
	<td>{$labels.audience}</td>
	<td width="50%" class="right">{html_inputField type="select" name="audid" selected=$audiences.selected options=$audiences.options}</td>
	{assign var="rowidx" value=$rowidx+1}
</tr>
<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	<td class="left"></td>
	<td>{$labels.collection}</td>
	<td width="50%" class="right">{html_inputField type="select" name="colid" selected=$collections.selected options=$collections.options}</td>
	{assign var="rowidx" value=$rowidx+1}
</tr>
<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	<td class="left"></td>
	<td>{$labels.material}</td>
	<td width="50%" class="right">{html_inputField type="select" name="matid" selected=$materials.selected options=$materials.options  onchange="doMaterialChange(this.value);"}</td>
...
{foreach from=$fields.material key="tidx" item="part"}
	...
	<td class="left" align="center"  width="8%">{$tidx}</td>
...
{foreach from=$fields.other key="tidx" item="part"}
	...
	{/if}
	<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	<td align="center" class="left" width="8%"><input type="button" value="{$tidx}" onclick="self.location='entry_marc_edit.php?f={$part.field->getFieldid()}'"/></td>
	<td width="35%" >{if $part.field->getIsRequired()}<sup>*</sup>{/if}{$part.desc}</td>
