...
<form id="entryeditform" method="post" action="entry_new_like.php">
...
{foreach from=$fields.general key="tidx" item="part"}
	...
	<td width="8%" align="center"  class="left">{$tidx}</td>
...
<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	<td class="left"></td>
	<td>{$collections.label}</td>
	<td width="50%" class="right">{html_inputField type="select" name="colid" selected=$collections.selected options=$collections.types}</td>
	{assign var="rowidx" value=$rowidx+1}
</tr>
<tr class="{if $rowidx is even}primary{else}alt1{/if}">
	<td class="left"></td>
	<td>{$materials.label}</td>
	<td width="50%" class="right">{html_inputField type="select" name="matid" selected=$materials.selected options=$materials.types  onchange="doMaterialChange(this.value);"}</td>
