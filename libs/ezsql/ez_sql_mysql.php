	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
		...
		{
			$this->_dbuser = OBIB_USERNAME;
			$this->_dbpassword = OBIB_PWD;
			$this->_dbname = OBIB_DATABASE;
			$this->_dbhost = OBIB_HOST;

			$this->cache_timeout = 24; // Note: this is hours

			// Specify a cache dir.
			// (1. You must create this dir. first!)
			// (2. Might need to do chmod 775)
			if ($this->cache_dir == false) // check we have not already set the path
				$this->cache_dir = str_replace('//','/',dirname(__FILE__).'/') .'../ezsql/cache';
		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost')
		{
			...
			else if ( ! $this->_dbConn = @mysql_connect($dbhost,$dbuser,$dbpassword,false) )
			{
		...
		*/
		...
		function select($dbname='')
		{
			...
			else if ( !@mysql_select_db($dbname,$this->_dbConn) )
			{
				// Try to get error supplied by mysql if not use our own
				if ( !$str = @mysql_error($this->_dbConn))
		...
		*/
		...
		function query($query)
		{
			...
			if ( ! isset($this->_dbConn) || ! $this->_dbConn )
			{
			...
			$this->result = @mysql_query($query,$this->_dbConn);
			...
			if ( preg_match("/^(insert|delete|update|replace)\s+/i",$query) )
			{
				...
				{
					$this->insert_id = @mysql_insert_id($this->_dbConn);
				}
